...
def run_issues(nation, issue_list):
    ...
    AIPurpose = open('NationFramework/AIPurpose.txt', 'r').read().replace('\n', '')
    constitution = open('NationFramework/Constitution.txt', 'r').read().replace('\n', '')
    ideas = open('NationFramework/ideas.txt', 'r').read().replace('\n', '')
    values = open('NationFramework/values.txt', 'r').read().replace('\n', '')
    news = open('NationFramework/News.txt', 'r').read().replace('\n', '')
    ...
    for issue in issue_list:
        ...
        if not os.path.exists(f"data/issue/{nation.NATION}/{(current_date)}"):
            os.makedirs(f"data/issue/{nation.NATION}/{(current_date)}")
        add_to_file(f"data/issue/{nation.NATION}/{(current_date)}/{issue['id']}.txt",
                    f"ISSUE:\n{issue['id']}\nTEXT:\n{issue['text']}\nOPTIONS:\n{options_str}")
        
        if not os.path.exists(f"data/outputs/{nation.NATION}/{(current_date)}"):
            os.makedirs(f"data/outputs/{nation.NATION}/{(current_date)}")
        add_to_file(f"data/outputs/{nation.NATION}/{(current_date)}/{issue['id']}.txt",
                    f"issuecomplete_response:\n{issuecomplete_response}")
        
        if not os.path.exists(f"data/analysis/{nation.NATION}/{(current_date)}"):
            os.makedirs(f"data/analysis/{nation.NATION}/{(current_date)}")
        add_to_file(f"data/analysis/{nation.NATION}/{(current_date)}/{issue['id']}.txt",
                    f"ANALYSIS:\n{analysis}")
